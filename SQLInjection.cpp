...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Creates an injection pattern to determine if digits = digits or characters = characters 
  std::regex injectionPattern("([0-9*]=[0-9*])|('(.*)[a-zA-Z](.*)'='(.*)[a-zA-Z](.*)')");
  std::smatch match;

  // Searches for a match in the SQL injection pattern and displays a warning message to the console
  if (std::regex_search(sql.begin(), sql.end(), match, injectionPattern) && match.size() > 1) {
      std::cout << "SQL injection suspected" << std::endl;
      return false;
  }
